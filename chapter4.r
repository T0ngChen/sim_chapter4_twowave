...
if (impute.id == 1){
  ...
  se = 0.9
  sp = 0.2
} else if (impute.id == 10){
  beta = c(-3.553709, 0, 1)
  se = 0.8
  sp = 0.1
} else if (impute.id == 11){
  beta = c(-3.379317, -0.5, 1)
  se = 0.8
  sp = 0.1
} else if (impute.id == 12){
  beta = c(-3.251742, -1, 1)
  se = 0.8
  sp = 0.1
}
...
integer.neyman = function(n.strata, NS, sample.size, upper){
  nc = max(upper + 1)
  s = 1:nc
  arr = matrix(rep(NS, each = nc)/sqrt(s*(s+1)), nrow = n.strata, byrow = T)
  arr.list = as.list(as.data.frame(t(arr)))
...
one.sim<-function(beta, N = 15000, se, sp, n = 400){
  ## generate data
...
sim.fun = function(){
  while(TRUE){
    df <- try(one.sim(beta = beta, se = se, sp = sp), silent=TRUE)
